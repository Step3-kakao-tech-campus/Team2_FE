...
const options = [ //이 부분을 나중에 api 요청으로 대체
  { value: '불멸의 철갑궁', label: '불멸의 철갑궁' },
  { value: '신성한 파괴자', label: '신성한 파괴자' },
];

export const getImage = (image: string | undefined) => {
    if (image === "LEVEL_BRONZE")
        return ("/assets/badge_bronze.png");
    if (image === "LEVEL_SILVER")
        return ("/assets/badge_silver.png");
    if (image === "LEVEL_GOLD")
        return ("/assets/badge_gold.png");
    return ("/assets/badge_star.png");
}

const TitleHoldCard: React.FC<TitleHoldCardProps> = ({ image1, image2, title, description, titleName }) => {
    ...
    const { isLoading, isError, data, error } = useQuery(
        ['userTitles', userId],
        () => userId ? titleSearchApi.getUserTitles(userId) : Promise.reject('No user ID provided')
    );
        ...
        <div className="titlehold_card">
            ...
            <div className='titlehold_card_title'>
                ...
                <div className='titlehold_card_description'>
                    <img className="titlehold_card_image2" src={image2} alt="이미지 2" />
                    <h4>{titleName}</h4>
                    <p>{description}</p>
                </div>
            </div>
